...
class YOLOv8nTransfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.01, backbone_lr_ratio=0.1):
        ...
        yolo_model_wrapper = YOLO('yolov8n.pt') 
        self._model = yolo_model_wrapper.model.to(self._device)
    ...
    def train(self, train_loader, val_loader, test_loader):
        """YOLOv8n 전용 학습 루프 (ultralytics YOLO API 사용)"""
        
        OpLog(f"YOLOv8n Training with ultralytics.YOLO API")
        OpLog(f"Strategy: {self._gubun}, Epochs: {self._epochs}, LR: {self._lr}")
        
        # ultralytics YOLO 모델은 .train() 메서드로 직접 학습
        # 이를 위해 데이터를 YOLO 형식으로 준비해야 함
        
        try:
            # 임시 데이터셋 경로 생성 (YOLO 형식 필요)
            yolo_data_dir = os.path.join(self._meta.base_dir, "yolo_temp_data")
            os.makedirs(yolo_data_dir, exist_ok=True)
            
            # YOLO 학습 시작
            # ultralytics YOLO API의 표준 학습 방식
            results = self._model.train(
                data=None,  # 커스텀 데이터로더 사용
                epochs=self._epochs,
                imgsz=640,  # YOLOv8 표준 이미지 크기
                batch=4,    # 배치 크기
                device=self._device,
                ...
                momentum=0.9,
                weight_decay=5e-4,
                amp=True if torch.cuda.is_available() else False  # Automatic Mixed Precision
            )
            
            OpLog("✓ YOLOv8n Training completed successfully")
            return results
            
        except Exception as e:
            OpLog(f"Error during YOLOv8n training: {e}")
            OpLog("Falling back to manual training loop...")
...
def Execute_Training(model_name=None, gubun=None, epochs=None, lr=None, backbone_lr_ratio=None, batchSize=None):
    ...
    파라미터:
        model_name (str): 모델명 ("SSD", "FasterRCNN", "RetinaNet", "SSDLite", "YOLOv8n")
                         기본값: "YOLOv8n" (파라미터 없으면 YOLOv8n 사용)
        gubun (str): 학습 전략 ("partial", "freeze", "full")
        epochs (int): 에포크 수
        lr (float): 학습률
        backbone_lr_ratio (float): 백본 학습률 비율
        batchSize (int): 배치 크기
    
    예시:
        Execute_Training()  # YOLOv8n 기본 설정으로 실행
        Execute_Training(model_name="SSD", epochs=10, lr=0.001)  # SSD 커스텀 설정
    """
    
    # 모델별 기본 파라미터 설정
    default_params = {
        "YOLOv8n": {"gubun": "partial", "epochs": 20, "lr": 0.01, "backbone_lr_ratio": 0.05, "batchSize": 16},
        "SSD": {"gubun": "partial", "epochs": 10, "lr": 0.001, "backbone_lr_ratio": 1.0, "batchSize": 8},
        "FasterRCNN": {"gubun": "partial", "epochs": 5, "lr": 0.005, "backbone_lr_ratio": 0.1, "batchSize": 4},
        "RetinaNet": {"gubun": "freeze", "epochs": 5, "lr": 0.001, "backbone_lr_ratio": 0.1, "batchSize": 8},
        "SSDLite": {"gubun": "partial", "epochs": 30, "lr": 0.001, "backbone_lr_ratio": 0.1, "batchSize": 16},
    }
    
    # 모델명이 없으면 YOLOv8n으로 설정
    if model_name is None:
        model_name = "YOLOv8n"
    
    # 모델이 기본 파라미터에 있는지 확인
    if model_name not in default_params:
        raise ValueError(f"Unknown model: {model_name}. Available models: {list(default_params.keys())}")
    
    # 기본 파라미터 적용
    defaults = default_params[model_name].copy()
    
    # 사용자 지정 파라미터로 기본값 덮어쓰기
    if gubun is not None:
        defaults["gubun"] = gubun
    if epochs is not None:
        defaults["epochs"] = epochs
    if lr is not None:
        defaults["lr"] = lr
    if backbone_lr_ratio is not None:
        defaults["backbone_lr_ratio"] = backbone_lr_ratio
    if batchSize is not None:
        defaults["batchSize"] = batchSize
    
    # 최종 파라미터 추출
    final_gubun = defaults["gubun"]
    final_epochs = defaults["epochs"]
    final_lr = defaults["lr"]
    final_backbone_lr_ratio = defaults["backbone_lr_ratio"]
    final_batchSize = defaults["batchSize"]
    
    Lines(f"Execute_Training Start for {model_name}")
    Lines(f"Parameters: gubun={final_gubun}, epochs={final_epochs}, lr={final_lr}, backbone_lr_ratio={final_backbone_lr_ratio}, batchSize={final_batchSize}")
    ...
    model = MakeModel(model_name, meta=meta, gubun=final_gubun, epochs=final_epochs, lr=final_lr, backbone_lr_ratio=final_backbone_lr_ratio)
...
def Trains():
    ...
    # 2. SSD300 VGG16
    # Execute_Training(model_name="SSD")
    
    # 3. FasterRCNN ResNet50
    # Execute_Training(model_name="FasterRCNN")
    
    # 4. RetinaNet ResNet50
    # Execute_Training(model_name="RetinaNet")
    
    # 5. SSDLite MobileNetV3
    # Execute_Training(model_name="SSDLite")

def Trains_All():
    """모든 모델 학습 (모든 모델 순차 실행)"""
    
    print("\n" + "="*100)
    print("1️⃣ YOLOv8n 학습 시작")
    print("="*100)
    Execute_Training()
    
    print("\n" + "="*100)
    print("2️⃣ SSD300 VGG16 학습 시작")
    print("="*100)
    Execute_Training(model_name="SSD")
    
    print("\n" + "="*100)
    print("3️⃣ FasterRCNN ResNet50 학습 시작")
    print("="*100)
    Execute_Training(model_name="FasterRCNN")
    
    print("\n" + "="*100)
    print("4️⃣ RetinaNet ResNet50 학습 시작")
    print("="*100)
    Execute_Training(model_name="RetinaNet")
    
    print("\n" + "="*100)
    print("5️⃣ SSDLite MobileNetV3 학습 시작")
    print("="*100)
    Execute_Training(model_name="SSDLite")
