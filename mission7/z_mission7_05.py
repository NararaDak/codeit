...
class TrainerUnified:
    def __init__(self, mode, model, optimizer, device, train_loader, val_loader, test_loader=None, scheduler=None, 
                 model_name="", gubun="", lr=0.0, epochs=0):
        """
        mode: 'det' or 'cls'
    ...
    def fit(self, epochs, save_dir=None):
        ...
        for epoch in range(1, epochs+1):
            ...
            if self.mode == "cls":
                ...
                print(f"[{now_str()}] Epoch {epoch} det train_loss={tr_loss:.4f} val_mAP@0.5={val_mAP:.4f} val_mAP@0.75={val_metric['mAP@0.75']:.4f} time={(time.time()-t0):.1f}s")
        ...
        if self.mode == "cls" and self.test_loader is not None:
            ...
            final_test_results = {"test_loss": test_loss, "test_acc": test_acc, "best_epoch": best_epoch}
