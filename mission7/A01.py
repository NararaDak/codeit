...
class SSD300VGG16Transfer(BasicTransfer):
    ...
    def train(self, train_loader=None, val_loader=None): # Loader가 없으면 내부에서 생성
        if train_loader is None:
            train_loader, val_loader, _ = GetLoader(custom_transform=self._transforms) # 자신의 Transform 전달

        optimizer, scheduler = self.getOptimizer()
        for epoch in range(self._epochs):
            self._model.train()
            total_loss = 0
            for images, targets in tqdm(train_loader, desc=f"Training E{epoch+1} ({self.getMyName()})"):
                images = [img.to(self._device) for img in images]
            ...
            # Validation & Save
            mAP = self.evalModel(val_loader, epoch)
            save_metrics_to_csv({'avg_loss': total_loss/len(train_loader), 'mAP': 0}, f"{self.getMyName()}_{self._gubun}", self._epochs, self._lr, epoch+1, "Train")
            save_metrics_to_csv({'avg_loss': 0, 'mAP': mAP}, f"{self.getMyName()}_{self._gubun}", self._epochs, self._lr, epoch+1, "Validation")
...
class FasterRCNNResNet50Transfer(BasicTransfer):
    ...
    def train(self, train_loader=None, val_loader=None):
        if train_loader is None:
            train_loader, val_loader, _ = GetLoader(custom_transform=self._transforms)
...
class RetinaNetResNet50Transfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1):
        ...
        # 1. COCO 가중치로 모델 로드 (91 클래스)
        model_coco = retinanet_resnet50_fpn(weights=self._weights).to(self._device)
        
        # 2. 번역 학습: backbone은 COCO 가중치 유지, head는 3 클래스로 재구성
        # RetinaNet 구조: backbone -> head (classification_head + bbox_regression_head)
        # Head의 마지막 Conv2d 레이어만 가중치 마이그레이션 (91 -> 3 클래스)
        num_anchors = model_coco.head.classification_head.num_anchors
        
        # backbone만 추출 후 새 모델 구성
        self._model = retinanet_resnet50_fpn(weights=None).to(self._device)  # 가중치 없이 구성
        
        # COCO 모델의 backbone 가중치를 새 모델에 복사
        self._model.backbone.load_state_dict(model_coco.backbone.state_dict())
        
        # Head의 classification layer 가중치 마이그레이션 (91 -> 3 클래스)
        try:
            old_cls_head = model_coco.head.classification_head
            new_cls_head = self._model.head.classification_head
            
            # 모든 레이어 가중치를 복사 (마지막 레이어는 채널 수 조정)
            for (old_name, old_module), (new_name, new_module) in zip(
                old_cls_head.named_modules(), new_cls_head.named_modules()
            ):
                if isinstance(new_module, torch.nn.Conv2d):
                    if hasattr(old_module, 'weight') and old_module.weight is not None:
                        ...
                        old_w = old_module.weight
                        new_w = new_module.weight
                        
                        # 마지막 Conv2d (output: 91 -> 3): 처음 3개 채널만 복사
                        if old_w.shape[0] == 91 and new_w.shape[0] == 3:
                            new_module.weight.data = old_w[:3, :, :, :].clone()
                            if old_module.bias is not None and new_module.bias is not None:
                                new_module.bias.data = old_module.bias[:3].clone()
                        elif old_w.shape == new_w.shape:
                            # 동일 크기면 복사
                            new_module.weight.data = old_w.clone()
                            if old_module.bias is not None:
                                new_module.bias.data = old_module.bias.clone()
        except Exception as e:
            OpLog(f"RetinaNet head weight migration warning: {e}", bLines=False)
...
class SSDLiteMobileNetV3Transfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1):
        ...
        from torchvision.models.detection import SSDLite320_MobileNetV3_Large_Weights
        from torchvision.models.detection import ssdlite320_mobilenet_v3_large
        # Weights & Model & Transform
        self._weights = SSDLite320_MobileNetV3_Large_Weights.DEFAULT # .DEFAULT로 수정
        self._transforms = self._weights.transforms()
        ...
        # SSDLite의 Head는 ModuleList로 되어 있어 첫 번째 conv의 input channel을 가져옴
        if hasattr(self._model.head.classification_head, 'module_list'): # 구버전 호환
             in_channels = [m[0].in_channels for m in self._model.head.classification_head.module_list]
        else: # 최신 버전 (일반적으로 리스트 형태의 채널 정보가 필요할 수 있음)
             # MobileNetV3 Large SSDLite 표준 채널: [960, 960, 960, 960, 960, 960] (Feature map 개수만큼)
             # 안전하게 Anchor Generator 개수와 맞춤
             num_anchors = self._model.anchor_generator.num_anchors_per_location()
             # SSDLite는 보통 모든 헤드에 대해 동일한 in_channels를 가짐 (마지막 feature map 채널)
             # 간단히 구현하기 위해 SSDHead를 재활용 (표준 SSD와 구조 유사)
             # SSDLite는 DepthwiseSeparableConv를 쓰지만, Transfer Learning에서는 
             # 일반 Conv(SSDClassificationHead)로 바꿔도 잘 동작함.
