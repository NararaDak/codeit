...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 반환: (image, target_dict) 형태로 반환하여 evaluate_model에서 안전하게 사용 가능하도록 함
...
class BasicTransfer(nn.Module):
    ...
    def visualize_prediction(self,image, prediction, classes):
        ...
        plt.axis("off")
        # Non-blocking draw then close (saving handled by testMode)
        plt.draw()
        plt.pause(0.1)
        plt.close()
...
Execute_Training(model_name="SSDLite", gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1, batchSize=16)
