...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms:
            image, _ = self.transforms(image, dummy_target) # 타겟은 무시하고 이미지만 변환
...
# ════════════════════════════════════════
def GetLoader(batchSize=8, custom_transform=None):
    meta = MyMeta()
...
class SSD300VGG16Transfer(BasicTransfer):
    ...
    def train(self, train_loader=None, val_loader=None): # Loader가 없으면 내부에서 생성
        if train_loader is None:
            train_loader, val_loader, _ = GetLoader(custom_transform=self.transforms) # 자신의 Transform 전달
...
class FasterRCNNResNet50Transfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1):
        ...
        self._model = fasterrcnn_resnet50_fpn(weights=self.weights).to(self._device)
    ...
    def train(self, train_loader=None, val_loader=None):
        if train_loader is None:
            train_loader, val_loader, _ = GetLoader(custom_transform=self.transforms)
...
class RetinaNetResNet50Transfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1):
        ...
        self.transforms = self.weights.transforms()
        # RetinaNet은 num_classes를 인자로 주어 모델을 새로 생성하는 것이 Head 교체에 용이함 (가중치는 로드됨)
        self._model = retinanet_resnet50_fpn(weights=self.weights, num_classes=self._num_classes).to(self._device)
...
class SSDLiteMobileNetV3Transfer(BasicTransfer):
    def __init__(self, meta=None, gubun="partial", epochs=10, lr=0.001, backbone_lr_ratio=0.1):
        ...
        self.transforms = self.weights.transforms()
        self._model = ssdlite320_mobilenet_v3_large(weights=self.weights).to(self._device)
...
# def Execute_Training(name,gubun ="partial", epochs =5,lr=0.001,backbone_lr_ratio=0.1):
#     # 1. 메타 정보 로드
#     meta = MyMeta()
...
#     # 3. 모델 생성
#     model = MakeModel(name, meta=meta, gubun=gubun, epochs=epochs, lr=lr, backbone_lr_ratio=backbone_lr_ratio)
#     # 4. 모델 학습
#     model.train(train_loader, val_loader)
    
# Execute_Training("SSDLite",1,1,1)
# Execute_Training("RetinaNet",1,1,1)
# Execute_Training("FasterRCNN",1,1,1)
# Execute_Training("SSD",1,1,1)
