...
class VarInfo():
    def __init__(self):
        self.mBaseDir = ""
        self.mTrainDir = ""
        self.mTrainCleaned_dir = ""
        self.mTestDir = ""
        self.mDevice = None
    def GetReadBaseDir(self):
        config = conf.ConfigParser()
        SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
        filePath = os.path.join(SCRIPT_DIR, "config.ini")
        config.read(filePath)
        baseDir = config["DIR_INFO"]["BASE_DIR"]
        return baseDir
    def GetDirs(self,baseDir):
        trainDir = f"{baseDir}/train" # Train data X.
        trainCleaned_dir = f"{baseDir}/train_cleaned" # Train data Y.(Label)
        testDir = f"{baseDir}/test" # Test data X.
        return trainDir,trainCleaned_dir,testDir
    def CheckVar(self):
        if( self.mBaseDir==""):
            self.mBaseDir= self.GetReadBaseDir()
            self.mTrainDir,self.mTrainCleaned_dir,self.mTestDir = self.GetDirs(self.mBaseDir)
        if self.mDevice == None:
            self.mDevice = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    def __call__(self):
        self.CheckVar()
        return self.mBaseDir,self.mTrainDir,self.mTrainCleaned_dir,self.mTestDir
    def GetBaseDir(self):
        return self.mBaseDir
    def GetTrainDir(self):
        return self.mTrainDir
    def GetTrainCleanedDir(self):
        return self.mTrainCleaned_dir
    def GetTestDir(self):
        return self.mTestDir
    def GetDevice(self):
        return self.mDevice
g_Vars = VarInfo()
g_Vars()
...
EPOCHS = 5
LEARN_RATE = 0.00001
MODEL_PATH = f"{g_Vars.GetBaseDir()}/modelfiles/{MODEL_CLASS_NAME}_{EPOCHS}_{LEARN_RATE}.pth"
...
class PairedImageDataset(Dataset):
    def __init__(self, InputTransform=None, LabelTransform=None):
        """
        ...
        """
        train_dir =g_Vars.GetTrainDir()
        train_cleaned_dir = g_Vars.GetTrainCleanedDir()

        self.train_image = loadImageDataset(train_dir, InputTransform)
        self.cleaned_image = loadImageDataset(train_cleaned_dir, LabelTransform)
...
class DenoisingModel(nn.Module):
    def __init__(self, pretrained_backbone, target_image_size=224):
        ...
        # DINOv2-base의 특징 크기는 768.
        # Vision Transformer의 출력은 [B, N_patches, 768] 형태.
        # 복원을 위해 특징을 이미지와 유사한 2D 형태로 변환하고 업샘플링.
        
        # 임시 복원 헤드: 768차원 특징을 받아 3채널 이미지로 변환 및 업샘플링
        # 이 부분은 DINOv2의 패치 크기와 이미지 크기에 따라 복잡한 Reshape 및 Transpose 과정이 필요
...
def ExtraTrain(pairedLoader, epochs, lr):
    ...
    pretrainedModel = GetExtractAutoMode().to(g_Vars.GetDevice()) # 모델을 장치로 이동
    # 1. 손실 함수 정의 (MSE Loss)
    loss_fn = nn.MSELoss() 
    # 2. 옵티마이저 정의 (Adam)
    ...
    with torch.no_grad():
        for trainImages_test, cleanImages_test in pairedLoader:
            trainImages_test = trainImages_test.to(g_Vars.GetDevice())
            ...
            cleanImages_test = cleanImages_test.to(g_Vars.GetDevice())
    ...
    for epoch in range(epochs):
        ...
        for trainImages, cleanedImages in pairedLoader:
            # 데이터를 장치로 이동
            trainImages = trainImages.to(g_Vars.GetDevice())
            cleanedImages = cleanedImages.to(g_Vars.GetDevice())
...
#EPOCHS = 5 
#LEARN_RATE = 0.00001
def Execute_Model(epochs,lr):
    train_dir = g_Vars.GetTrainDir()
    train_cleaned_dir = g_Vars.GetTrainCleanedDir()
    test_dir = g_Vars.GetTestDir()
    #────────────────────────────────────────
    # 이미지 불러 오기.
    #────────────────────────────────────────
    ...
    # 데이터셋 생성
    pairedDataset = PairedImageDataset( g_transforms,g_transforms)
    ...
    #───────────────────────────────────────
    Lines("모델 훈련을 시작합니다.")
    return ExtraTrain(pairedLoader, epochs, lr)
...
# 테스트
    model.eval()
    testDataset = loadImageDataset(g_Vars.GetTestDir(), g_transforms) 
    testLoader = DataLoader(testDataset, batch_size=16, shuffle=False, num_workers = 0)
    with torch.no_grad():
        nIndex = 1
        for images in testLoader:
            images_batch = images.to(g_Vars.GetDevice()) # 원본 이미지
...
def saveResultFile(nIndex, images, outputs, className, epoches, learnRate):
    ...
    for i in range(images_np.shape[0]):
        ...
        # 저장 경로 설정
        imagedir = f"{g_Vars.GetBaseDir()}/model_result_image"
...
def GetModel():
    ...
    if os.path.exists(MODEL_PATH):
        ...
        # 저장된 모델 전체를 로드합니다.
        model = torch.load(MODEL_PATH, map_location= g_Vars.GetDevice(), weights_only=False)
        return model
    else:
        print(f"'{MODEL_PATH}' 파일이 없어, 모델 훈련을 시작합니다.")
        # 모델 훈련
        model = Execute_Model(EPOCHS,LEARN_RATE)
        # 훈련된 모델 저장
...
def Execute_fn():
    ...
    evalModel(model_class,MODEL_CLASS_NAME,EPOCHS,LEARN_RATE)
